...
/**
 * InputHandler that performs text lowercasing.
 * Converts all text to lowercase for case-insensitive processing.
 */
export class LowercaseInputHandler extends BaseInputHandler {
  async handle(input: InputHandlerInput): Promise<InputHandlerOutput> {
    ...
    // Apply lowercasing
    const lowercased = controlCharsStripped.toLowerCase();
    if (lowercased !== controlCharsStripped) {
      normalisationLog.push({
        name: 'lowercase_normalisation',
        params: {},
      });
    }

    // Normalise whitespace (collapse multiple spaces, trim)
    const whitespaceNormalised = lowercased
      .replace(/\s+/g, ' ')  // Collapse multiple whitespace to single space
      .trim();                // Remove leading/trailing whitespace

    if (whitespaceNormalised !== lowercased) {
      normalisationLog.push({
